...
class HeteGAT_multi(BaseGAttN):
    def inference(inputs_list, nb_classes, nb_nodes, training, attn_drop, ffd_drop,
                  ...
                  mp_att_size=128):
        embed_list = []
        for inputs, bias_mat in zip(inputs_list, bias_mat_list): #Parcours la liste des features de chaque noeuds et les meta-path
            attns = []
            jhy_embeds = []
            for _ in range(n_heads[0]):#Multi-head attention
                attns.append(layers.attn_head(inputs, bias_mat=bias_mat,
                                              out_sz=hid_units[0], activation=activation,
                                              in_drop=ffd_drop, coef_drop=attn_drop, residual=False))#node level aggregating 
            h_1 = tf.concat(attns, axis=-1)

            for i in range(1, len(hid_units)):#Boucle pour les autres couche si il y a d'autre couche dans le réseaux
                h_old = h_1
                attns = []
                for _ in range(n_heads[i]):
                    attns.append(layers.attn_head(h_1, bias_mat=bias_mat,
        ...
        final_embed, att_val = layers.SimpleAttLayer(multi_embed, mp_att_size,
                                                     time_major=False,
                                                     return_alphas=True)# Calcul les Beta et les noeuds finaux

        out = []
        for i in range(n_heads[-1]): #Obtention des classes via les noeuds finaux, pour la loss
